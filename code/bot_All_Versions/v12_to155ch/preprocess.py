...
def saveData():
    ...
    # 离散通道缩放为整数 → int8 精确保存 (零信息损失)
    #   ch32-35  WALL:     tileWall[p]/21  → ×21 (值 0~21)
    #   ch149    TILE_FROM: tileFrom/3     → ×3  (值 0~3)
    #   ch152-54 HAND_SIZE: handSize/14    → ×14 (值 0~14)
    #   其余 147 通道: 0/1 one-hot → 直接存
    obs_stack[:, 32:36, :, :] *= 21
    obs_stack[:, 149, :, :] *= 3
    obs_stack[:, 152:155, :, :] *= 14
    obs_i8 = np.round(obs_stack).astype(np.int8)
    np.savez_compressed('data/%d.npz'%matchid
        , obs = obs_i8
