...
class FeatureAgent(MahjongGBAgent):
    ...
    OFFSET_OBS = {
        ...
        'CUR_TILE': 148,   # 1 channel:  当前待决策牌 one-hot
        'TILE_FROM': 149,  # 1 channel:  牌源方归一化标量
        'WALL_LAST': 150,  # 1 channel:  牌墙最后一张标志
        'IS_ABOUT_KONG': 151,  # 1 channel: 刚杠标志
        'HAND_SIZE': 152,  # 3 channels: 152-154 对手手牌数
    }
    ...
    def _update_obs(self):
        ...
        # 11. 对手手牌数归一化 (Ch 152-154, 相对位置 1,2,3)
        for i in range(1, 4):
    ...
    def request2obs(self, request: str):
        ...
        if t[0] == 'Draw':
            ...
            self._update_obs()
            self.isAboutKong = False  # 必须在 _update_obs() 之后清除，确保 ch151 编码正确
            return self._make_obs()
