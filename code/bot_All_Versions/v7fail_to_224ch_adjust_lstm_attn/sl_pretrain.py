...
class SLDataset(Dataset):
    def __init__(self, data_dir='data', begin=0.0, end=1.0):
        ...
        offsets = [0]
        for m in self.match_samples: offsets.append(offsets[-1] + m)
        self.offsets = offsets
        self.samples = sum(self.match_samples)
        self.data_dir = data_dir
        print(f'Dataset: {len(self.match_samples)} matches, {self.samples} samples')
...
if __name__ == '__main__':
    ...
    batch_size = 4096 if device.type == 'npu' else 2048
