    ...
    def run(self):
        ...
        try:
            ...
            env = MahjongGBEnv(config={
                'agent_clz': FeatureAgent, 'duplicate': True,
                'reward_scale': self.config.get('reward_scale', 48.0),
            })
            ...
            for episode in range(self.config['episodes_per_actor']):
                ...
                obs = env.reset()
                ep_data = {a: {'obs': [], 'mask': [], 'act': [], 'rew': [], 'val': [], 'logp': []}
                ...
                while not done and not self.stop_event.is_set() and steps < 200:
                    ...
                    for agent_name in obs:
                        ...
                        with torch.no_grad():
                            ...
                            log_prob = lp[0, action].item()
                        actions[agent_name] = action
                        ep_data[agent_name]['act'].append(action)
                        ep_data[agent_name]['val'].append(values.squeeze(-1).item())
                        ep_data[agent_name]['logp'].append(log_prob)

                    next_obs, rewards, done = env.step(actions)
                    steps += 1
                    # 只给本轮行动的 agent 记 reward，避免 key 错位
                    for agent_name in actions: ep_data[agent_name]['rew'].append(rewards.get(agent_name, 0))
                    obs = next_obs
                ...
                for agent_name, data in ep_data.items():
                    ml = min(len(data['obs']), len(data['act']), len(data['rew']), len(data['val']), len(data['logp']))
                    if ml == 0: continue
                    ...
                    logp_arr = np.array(data['logp'][:ml], dtype=np.float32)
                    ...
                    # normalize advantages (only for policy gradient)
                    # reward 已在 env 层缩放，advantage 量级天然合理，无需硬截断
