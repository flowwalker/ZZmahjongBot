...
def parse_args():
    p = argparse.ArgumentParser(description='Gated fusion RL Training')
...
def main():
    ...
    config = {
        ...
        'model_pool_size': 20, 'model_pool_name': 'vultra-pool',
        ...
        'use_aux_tasks': args.use_aux, 'device': args.device, 'ckpt_save_interval': 300,
    ...
    print(f'[Main] Gated fusion RL | Actors:{args.num_actors} | Dev:{args.device} | Aux:{args.use_aux}')
    ...
    actors, learner = [], None
    try:
        learner = Learner(config, replay_buffer); learner.start(); time.sleep(2)
