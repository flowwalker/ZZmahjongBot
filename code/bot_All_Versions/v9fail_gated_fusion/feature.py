...
class FeatureAgent(MahjongGBAgent):
    ...
    OFFSET_ACT = {'Pass': 0, 'Hu': 1, 'Play': 2, 'Chi': 36,
                  'Peng': 99, 'Gang': 133, 'AnGang': 167, 'BuGang': 201}
    TILE_LIST = [*(f'W{i+1}' for i in range(9)), *(f'T{i+1}' for i in range(9)),
                 *(f'B{i+1}' for i in range(9)), *(f'F{i+1}' for i in range(4)),
    ...
    def _update_obs(self):
        self.obs.fill(0)
        self._update_base_features()
        self._update_v2_features()
        self._update_v3_features()
    ...
    def _count_taatsu(self, hand_array):
        ...
        for suit_idx in range(3):
            ...
            for i in range(8):
                if arr[i] > 0 and arr[i + 1] > 0: taatsu += 0.5
            for i in range(7):
                if arr[i] > 0 and arr[i + 2] > 0: taatsu += 0.3
        return taatsu

    def _calc_shanten_features(self, hand_array):
        n = hand_array.sum()
        if n == 0: return {'current': 8, 'if_discard': np.full(34, 8.0, dtype=np.float32)}
        ...
        for idx in range(34):
            if hand_array[idx] == 0: continue
            sim = hand_array.copy(); sim[idx] -= 1
            st = self._count_taatsu(sim); sm = self._count_mentsu(sim)
            sg = sm + min(st, 5) * 0.5
    ...
    def _calc_wait_type(self, hand_array, wall_remaining):
        ...
        for idx in range(34):
            ...
            if wall_remaining[tile] == 0: continue
            if hand_array[idx] >= 2: single[idx] = 1.0
            if tile[0] in 'WTB':
                num = int(tile[1]); suit = tile[0]
                sb = {'W': 0, 'T': 9, 'B': 18}[suit]
                if 2 <= num <= 7:
                    if hand_array[sb + num - 2] > 0 and hand_array[sb + num - 1] > 0: double_sided[idx] = 1.0
                    if hand_array[sb + num] > 0 and hand_array[sb + num + 1] > 0: double_sided[idx] = 1.0
                if num == 3 and hand_array[sb] > 0 and hand_array[sb + 1] > 0: edge[idx] = 1.0
                if num == 7 and hand_array[sb + 7] > 0 and hand_array[sb + 8] > 0: edge[idx] = 1.0
            n_pairs = sum(1 for c in hand_array if c >= 2)
            if n_pairs >= 2 and hand_array[idx] >= 2: pair_wait[idx] = 1.0
    ...
    def _calc_precise_danger(self, hand_array, wall_remaining):
        ...
        for idx in range(34):
            ...
            if seen >= 4: danger[idx] = 0.0; continue
            suji = 0.0
            if tile[0] in 'WTB':
                num = int(tile[1]); suit = tile[0]
                ...
                if si is not None:
                    for p in range(1, 4):
                        if st in self.history[p]: suji = 0.3; break
            gen = 0.0
            for p in range(1, 4):
                if tile in self.history[p]: gen = max(gen, 0.4)
            ol = 0.0
            for p in range(1, 4):
                if len(self.packs[p]) >= 3: ol = max(ol, 0.3)
    ...
    def _calc_yaku_progress(self, hand_array):
        ...
        for idx in range(27):
            if hand_array[idx] > 0 and idx not in yaochu: all_simples[idx] = 0.5
            num = (idx % 9) + 1
            if 2 <= num <= 8 and hand_array[idx] > 0:
                pinfu[idx] = 0.3
                if idx % 9 > 0 and hand_array[idx - 1] > 0: pinfu[idx] = 0.7
                if idx % 9 < 8 and hand_array[idx + 1] > 0: pinfu[idx] = max(pinfu[idx], 0.7)
        for s, suit in enumerate(['W', 'T', 'B']):
            ...
            if sc / total > 0.7:
                for i in si: half_flush[i] = full_flush[i] = sc / total
    ...
    def _calc_hand_structure(self, hand_array):
        ...
        taatsu = 0
        for s in range(3):
            base = s * 9; a = hand_array[base:base + 9]
            for i in range(8):
                if a[i] > 0 and a[i+1] > 0: taatsu += 1
            for i in range(7):
                if a[i] > 0 and a[i+2] > 0: taatsu += 1
        koutsu = sum(1 for c in hand_array if c >= 3)
        shuntsu = 0
        for s in range(3):
            base = s * 9; a = hand_array[base:base + 9].copy()
            for i in range(7):
                while a[i] > 0 and a[i+1] > 0 and a[i+2] > 0:
                    shuntsu += 1; a[i] -= 1; a[i+1] -= 1; a[i+2] -= 1
    ...
    def _calc_opponent_inference(self):
        ...
        for p in range(1, 4):
            ...
            for d in discards: suit_count[d[0]] += 1
            for idx in range(34):
                tile = self.TILE_LIST[idx]; suit = tile[0]; td = suit_count.get(suit, 0)
                if td <= 2 and len(discards) > 5: likely_need[idx] += 0.15
                if tile in discards:
                    genbutsu[idx] = 1.0
                    if suit in 'WTB':
                        num = int(tile[1])
                ...
                if suit in 'WTB':
                    num = int(tile[1])
                    for d in discards:
                        if d[0] == suit and abs(int(d[1]) - num) <= 1: likely_discard[idx] += 0.1
    ...
    def _calc_game_phase(self):
        ...
        if tp < 12: return {'early': 1.0, 'mid': 0.0, 'late': 0.0, 'all_last': 0.0}
        elif tp < 36: return {'early': 0.0, 'mid': 1.0, 'late': 0.0, 'all_last': 0.0}
        elif tp < 54: return {'early': 0.0, 'mid': 0.0, 'late': 1.0, 'all_last': 0.0}
        else: return {'early': 0.0, 'mid': 0.0, 'late': 0.0, 'all_last': 1.0}

    def _hand_to_array(self):
        arr = np.zeros(34, dtype=np.int32)
        for t in self.hand: arr[self.OFFSET_TILE[t]] += 1
        return arr
    def _shown_to_array(self):
        arr = np.zeros(34, dtype=np.int32)
        for t, c in self.shownTiles.items(): arr[self.OFFSET_TILE[t]] = c
        return arr
    def _calc_tile_efficiency(self, ha, wr):
        eff = np.zeros(34, dtype=np.float32)
        for idx in range(34):
            tile = self.TILE_LIST[idx]; cnt = ha[idx]
            if cnt == 0: continue
            pp = min(cnt, 3) / 3.0; suit = tile[0]; num = int(tile[1]); sp = 0.0; ip = 0.0
            if suit in 'WTB':
                nb = []
                for d in range(-2, 3):
                    n = num + d
                    if 1 <= n <= 9: nb.append(ha[self.OFFSET_TILE[suit + str(n)]])
                    else: nb.append(0)
                ss = []
                for start in range(max(0, num - 2), min(7, num)):
                    if all(nb[start + k - (num - 2)] > 0 for k in range(3)): ss.append(1.0)
                    elif sum(1 for k in range(3) if nb[start + k - (num - 2)] > 0) >= 2: ss.append(0.5)
                sp = max(ss) if ss else 0.0
                ha_adj = False
                for d in (-1, 1):
                    n = num + d
                    if 1 <= n <= 9:
                        if ha[self.OFFSET_TILE[suit + str(n)]] > 0: ha_adj = True; break
                if not ha_adj and cnt == 1: ip = 0.3
            if suit in 'FJ':
                pb = min(cnt, 2) / 2.0; eff[idx] = pb * 0.8
            else:
                eff[idx] = max(0.0, max(pp, sp) - ip)
            rem = wr[tile]; af = min(rem / 2.0, 1.0); eff[idx] *= (0.3 + 0.7 * af)
        return np.clip(eff, 0, 1)
    def _calc_tile_safety(self, wr):
        s = np.ones(34, dtype=np.float32) * 0.5
        for idx in range(34):
            tile = self.TILE_LIST[idx]; ts = tile[0]; tn = int(tile[1])
            if ts in 'FJ':
                dc = sum(1 for p in range(1, 4) for t in self.history[p] if t == tile)
                if dc >= 1: s[idx] = 0.7 + 0.15 * min(dc, 2)
                ...
                for p in range(1, 4):
                    for t in self.history[p]:
                        if t[0] == ts and abs(int(t[1]) - tn) <= 2: nd += 1
                if nd >= 4: s[idx] = 0.8
                ...
                for d in range(-2, 3):
                    ...
                    if 1 <= n <= 9:
                        tr += 1
                        if self.shownTiles.get(ts + str(n), 0) > 0: rv += 1
                if tr > 0 and rv / tr > 0.6: s[idx] = max(s[idx], 0.75)
            rem = wr[tile]
            if rem == 0: s[idx] = 1.0
    ...
    def _calc_opponent_risk(self):
        ...
        for p in range(1, 4):
            ...
            for idx in range(34):
                tile = self.TILE_LIST[idx]; suit = tile[0]
                dr = sdc.get(suit, 0) / {'W': 9, 'T': 9, 'B': 9, 'F': 4, 'J': 3}.get(suit, 9)
                if dr < 0.1 and td > 5: risk[idx] += 0.15
                if suit in 'WTB':
                    num = int(tile[1])
                    for t in self.history[p]:
                        if t[0] == suit and abs(int(t[1]) - num) <= 1: risk[idx] += 0.05; break
            mc = len(self.packs[p])
            if mc >= 3: risk += 0.05 * (mc - 2)
        return np.clip(risk, 0, 1)
    def _calc_yaku_potential(self, ha):
        f = [np.zeros(34, dtype=np.float32) for _ in range(8)]
        for idx in range(27):
            num = (idx % 9) + 1
            if 2 <= num <= 8 and ha[idx] > 0: f[0][idx] = 0.5 + 0.5 * min(ha[idx], 2) / 2.0
        for idx in range(34):
            if ha[idx] >= 2: f[1][idx] = min(ha[idx], 3) / 3.0
        for suit in ['W', 'T', 'B']:
            si = [self.OFFSET_TILE[suit + str(i)] for i in range(1, 10)]
            sc = sum(ha[i] for i in si); total = max(ha.sum(), 1)
            if sc / total > 0.7:
        ...
        for idx in range(34):
            if ha[idx] > 0 and ha[idx] < 2 and self.TILE_LIST[idx][0] in 'WTB' and 2 <= int(self.TILE_LIST[idx][1]) <= 8: f[5][idx] = 0.4
        if len(self.packs[0]) == 0:
            for idx in range(34):
                if ha[idx] > 0: f[6][idx] = 0.3
        for idx in range(34):
            if ha[idx] == 4 or (ha[idx] == 3 and self.shownTiles.get(self.TILE_LIST[idx], 0) == 0): f[7][idx] = 0.7
        return f
    ...
    def get_aux_labels(self):
        ...
        si = self._calc_shanten_features(ha)
        ph = self._calc_game_phase()
        phase_idx = 0 if ph['early'] else (1 if ph['mid'] else (2 if ph['late'] else 3))
        return {
            'win_prob': self._estimate_tenpai_chance(ha),
            'shanten': float(si['current']),
    ...
    def request2obs(self, request: str):
        ...
        if t[2] == 'Gang':
            ...
            return None
        if t[2] == 'AnGang':
            # FIX: Botzone may send 'Player p AnGang' without tile
            tile = 'CONCEALED' if p else (t[3] if len(t) > 3 else 'CONCEALED')
    ...
    def _can_hu(self, winTile: str, isSelfDrawn: bool = False, isAboutKong: bool = False) -> bool:
        try:
            # shownTiles 只追踪外部可见牌，不包含手牌
            # 当手牌中有3张+别家打出1张时，原版(shownTiles+isSelfDrawn)==4 错误地返回False
            # 正确计算：shownTiles(外部) + hand.count(手牌中) == 4
