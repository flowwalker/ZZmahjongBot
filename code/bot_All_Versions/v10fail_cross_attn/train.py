"""RL training entry — GAE + uuid pool"""
import os, time, argparse, signal, uuid
...
def parse_args():
    p = argparse.ArgumentParser(description='Cross-attention RL Training')
...
def main():
    ...
    # uuid pool name prevents conflicts between runs
    pool_name = 'monster-pool-' + uuid.uuid4().hex[:6]
    config = {
        ...
        'use_aux_tasks': args.use_aux, 'device': args.device, 'ckpt_save_interval': 3600,
    ...
    print(f'[Main] Cross-attention RL | Actors:{args.num_actors} | Dev:{args.device} | Pool:{pool_name}')
