"""
国标麻将 Botzone Bot — 148通道 BiLSTM+SelfAttention 单模型版

注意：本文件与 bot_better_performance 目录对应的是 v6-era 单模型，
      并非 v19/v20 的 3模型投票版本（后者见 bot_best_performance / bot_final_upload）。
      接口严格对齐 can_do (真真正正可用的参考版本)。
"""

import os
# Limit threads to avoid timeouts in the Botzone sandbox.
...
if __name__ == '__main__':
    ...
    for p in [os.path.join(os.path.dirname(__file__), 'model_final.pt'),
              'model_final.pt',
              'data/model_final.pt',
              '/data/mahjong/model_final.pt']:
        if os.path.exists(p):
            ckpt = torch.load(p, map_location='cpu')
            if isinstance(ckpt, dict) and 'model' in ckpt:
                ckpt = ckpt['model']  # Extract weights
            try:
