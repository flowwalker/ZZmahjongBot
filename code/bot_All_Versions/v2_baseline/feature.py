...
class FeatureAgent(MahjongGBAgent):
    ...
    def _update_hand_embedding(self):
        ...
        for tile, cnt in d.items():
            self.obs[self.OFFSET_OBS['HAND']:self.OFFSET_OBS['HAND']+cnt,
                     self.OFFSET_TILE[tile]] = 1
        # 弃牌 (ch6-9)
        for pi in range(4):
            ch = self.OFFSET_OBS['DISCARD'] + pi
            for tile in self.history[pi]:
                idx = self.OFFSET_TILE[tile]
                self.obs[ch, idx] = min(4, self.obs[ch, idx] + 1)
        # 鸣牌 (ch10-13)
        for pi in range(4):
            ch = self.OFFSET_OBS['MELD'] + pi
            for pkt, tile, _ in self.packs[pi]:
                ...
                if idx is not None:
                    if pkt == 'CHI':
                        ...
                        for dn in (-1, 0, 1):
                            ...
                            if i2 is not None: self.obs[ch, i2] = 1
                    else:
                        self.obs[ch, idx] = 4 if pkt == 'GANG' else 3
        # 已见牌 (ch14)
        for tile, cnt in self.shownTiles.items():
            self.obs[self.OFFSET_OBS['SHOWN'], self.OFFSET_TILE[tile]] = cnt / 4.0
        # 牌墙 (ch15)
        self.obs[self.OFFSET_OBS['WALL'], :] = self.tileWall[0] / 21.0
