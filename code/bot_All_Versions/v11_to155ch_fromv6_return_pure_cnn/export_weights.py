...
def export(ckpt_path: str) -> str:
    ...
    # 如果当前机器没有 torch_npu，也能正常加载 (PyTorch 会自动回退存储层)
    ...
    # 逐层确保 CPU + contiguous (NPU tensor 可能非连续)
    clean = {}
    ...
    out_path = os.path.join(os.path.dirname(ckpt_path) or '.', 'model_final_pure_cnn.pt')
