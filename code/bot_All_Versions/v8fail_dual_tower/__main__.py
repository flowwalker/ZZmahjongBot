"""国标麻将 Botzone Bot — V5V6 合并版 (DualTower + MultiTask)"""
...
if __name__ == '__main__':
    ...
    for p in [os.path.join(os.path.dirname(__file__), 'model_final.pt'),
              'model_final.pt', 'data/model_final.pt', '/data/mahjong/model_final.pt']:
        if os.path.exists(p):
            try:
    ...
    while True:
        ...
        if t[0] == '0':
            ...
            with torch.no_grad():
                logits = model(state, mode='policy')  # 只推理策略塔
            ...
            if t[2] == 'DRAW':
                ...
                if p == seatWind: print('PASS')
                    ...
                    with torch.no_grad(): logits = model(state, mode='policy')
                    response = agent.action2response(logits.numpy().flatten().argmax())
                    print('HU' if response == 'Hu' else 'PASS')
            else:
                zimo = False
                ...
                if p == seatWind: print('PASS')
                    ...
                    if r[0] == 'Hu': print('HU')
                        ...
                        with torch.no_grad(): logits2 = model(state2, mode='policy')
