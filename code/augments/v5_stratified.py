...
def get_transforms_by_level(level):
    ...
    Levels:
        ...
        24  — digital symmetry(2) + suit(3) + honor(2) + wind(2) = 24
               selects sp<3, hp<2, wp<2
    ...
    for k in range(288):
        ...
        if level == 2:
            ...
            raise ValueError(
                f'Unknown level: {level}. Valid: 2, 12, 24, 72, 288')

    return [all_tfs[k] for k in keep]


def get_stratified_random_transform():
    """
    四维度分层随机: 每维度各随机取 1 个值，组合成 1 个变换。

    ds ∈ {0,1}, sp ∈ {0..5}, hp ∈ {0..5}, wp ∈ {0..3}
    index = ds*144 + sp*24 + hp*4 + wp

    保证每次采样覆盖全部 4 个变换维度，而非从 288 中盲目随机。
    """
    all_tfs = get_transforms()
    ds = np.random.randint(0, 2)
    sp = np.random.randint(0, 6)
    hp = np.random.randint(0, 6)
    wp = np.random.randint(0, 4)
    idx = ds * 144 + sp * 24 + hp * 4 + wp
    return all_tfs[idx]


def get_stratified_dual_suit_transforms():
    """
    四维度分层: 花色取2个，其余各取1个，产出2个变换。

    ds ∈ {0,1}, sp(2个无放回) ∈ {0..5}, hp ∈ {0..5}, wp ∈ {0..3}

    两个变换只有在花色排列上不同，增加花色维度的训练覆盖。
    """
    all_tfs = get_transforms()
    ds = np.random.randint(0, 2)
    sp1, sp2 = np.random.choice(6, size=2, replace=False)
    hp = np.random.randint(0, 6)
    wp = np.random.randint(0, 4)
    idx1 = ds * 144 + sp1 * 24 + hp * 4 + wp
    idx2 = ds * 144 + sp2 * 24 + hp * 4 + wp
    return [all_tfs[idx1], all_tfs[idx2]]


def get_random_transforms(n=4, seed=None):
    """
    Return n randomly selected transforms from the full 288 pool,
    always including the identity transform (index 0).

    Useful for per-epoch diversity: each epoch the model sees different
    transforms, but always at most n× the compute cost.

    Args:
        n: number of transforms to return (default 4)
        seed: optional random seed for reproducibility

    Returns:
        list of n transform dicts, where transforms[0] is always identity
    """
    all_tfs = get_transforms()
    rng = np.random.RandomState(seed)
    # Pick n-1 random transforms from indices 1..287 (exclude identity)
    random_indices = rng.choice(287, size=n - 1, replace=False) + 1
    # Identity first, then random
    return [all_tfs[0]] + [all_tfs[i] for i in random_indices]
