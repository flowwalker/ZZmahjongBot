...
class MahjongGBEnv():
    ...
    def __init__(self, config):
        ...
        assert issubclass(self.agentclz, MahjongGBAgent), "agent_clz must be a subclass of MahjongGBAgent!"
        self.duplicate = config.get('duplicate', True)
        self.variety = config.get('variety', -1)
        self.r = random.Random()
        self.reward_scale = config.get('reward_scale', 48.0)
    ...
    def _reward(self):
        # 始终为4人返回 reward，避免调用方按 self.obs key 取 reward 导致错位
        if self.reward: return {self.agent_names[i] : self.reward[i] for i in range(4)}
        return {self.agent_names[i] : 0 for i in range(4)}
    ...
    def _checkMahjong(self, player, isSelfDrawn = False, isAboutKong = False):
        try:
            ...
            s = self.reward_scale
            self.reward = [r / s for r in self.reward]
            self.done = True
